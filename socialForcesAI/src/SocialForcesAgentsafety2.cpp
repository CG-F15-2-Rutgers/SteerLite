...
*/
	...
	std::ifstream ifs;
	ifs.open("testcasename.txt");
	getline(ifs, testcaseName);
	ifs.close();

	if (testcaseName.find("hallway-two-way") != std::string::npos) { // DONE
		//runLongTermPlanning();
		AStarPlanner planner;
		std::vector<Util::Point> agentPath;
		planner.computePath(agentPath, this->position(), _goalQueue.front().targetLocation, gSpatialDatabase);

		SteerLib::AgentGoalInfo finalGoal = _goalQueue.front();
		_goalQueue.pop();
		int size = agentPath.size();
		for (int i = 0; i < size; ++i)
		{
			SteerLib::AgentGoalInfo goal;
			goal.targetLocation = agentPath.front();
			_goalQueue.push(goal);
			agentPath.erase(agentPath.begin());
		}
		_goalQueue.push(finalGoal);
	}
	else if (testcaseName.find("hallway-four-way-rounded-roundabout") != std::string::npos) { // DONE
		runLongTermPlanning();
	}
	else if (testcaseName.find("maze") != std::string::npos) { // DONE
		AStarPlanner planner;
		std::vector<Util::Point> agentPath;
		planner.computePath(agentPath, this->position(), _goalQueue.front().targetLocation, gSpatialDatabase);
		...
		if (position().z >= 0.0f) {
			...
			{
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
			else if (position().x >= 30.0f && position().x < 40.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
			...
			else if (position().x >= 40.0f && position().x < 50.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(position().x+35, 0, position().z - 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
			...
			else if (position().x >= 50.0f && position().x < 60.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(position().x + 60, 0, position().z - 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
			...
			else if (position().x >= 60.0f && position().x < 70.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(position().x + 85, 0, position().z - 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
			...
			else if (position().x >= 70.0f && position().x < 80.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(position().x + 110, 0, position().z - 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
			...
			else if (position().x >= 80.0f && position().x < 90.0f) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(position().x + 125, 0, position().z - 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, .82);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
				...
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(200, 0, .75);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
			...
			if (position().x >= 20.0f && position().x < 30.0f)
			{
				...
				goal.targetLocation = Util::Point(position().x + 125, 0, position().z + 3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(20, 0, -.68);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
				...
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(200, 0, -.70);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
		...
		if (position().x == -10.0f) {
			SteerLib::AgentGoalInfo goal;
			goal.targetLocation = Util::Point(2,0,0.8);
			_goalQueue.push(goal);
			_goalQueue.push(finalGoal);
			runLongTermPlanning();
		}
			...
			SteerLib::AgentGoalInfo goal;
			goal.targetLocation = Util::Point(17,0,-0.8);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(1, 0, -0.8);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(1, 0, 1.2);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(-2, 0, 0.8);
			_goalQueue.push(goal);
			_goalQueue.push(finalGoal);
		...
		if (position().x == -10.0f) {
			SteerLib::AgentGoalInfo goal;
			goal.targetLocation = Util::Point(-14, 0, 2.5);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(-5, 0, 2.5);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(-2, 0, 2.5);
			_goalQueue.push(goal);
			goal.targetLocation = Util::Point(0, 0, -.5);
			_goalQueue.push(goal);
			_goalQueue.push(finalGoal);
			runLongTermPlanning();
		}
			...
			SteerLib::AgentGoalInfo goal;
			goal.targetLocation = Util::Point(0, 0, .4);
			_goalQueue.push(goal);
			_goalQueue.push(finalGoal);
			runLongTermPlanning();
		}
		...
		if (position().x == -10.0f) {
			if (position().z == -0.5 ) { // LEFTIE
				...
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-12, 0, 0);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(0, 0, -.5);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
		...
		if (position().z == -34.2f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(10, 0, -40);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
		...
		else if (position().z == -30.8f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -30);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -38);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(5, 0, -38);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
				...
				goal.targetLocation = Util::Point(-3, 0, -38);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(6, 0, -38);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
		...
		else if (position().z == -24.0f) {
			
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -25);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -30);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -38);
		...
		else if (position().z == -20.6f) {
		

			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -20);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -25);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -30);
		...
		else if (position().z == -13.8f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -12);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -15);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -20);
		...
		else if (position().z == -10.4f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -9);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -12);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -15);
			...
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -6);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -9);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -12);
		...
		else if (position().z == -3.6f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, -3);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -6);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, -9);
			...
			if (position().x < -4) {
				...
				goal.targetLocation = Util::Point(-3, 0, 15);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 19);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 30);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(10, 0, 38);
				_goalQueue.push(goal);
				_goalQueue.push(finalGoal);
				runLongTermPlanning();
			}
		...
		else if (position().z == 23.6f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, 21);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 25);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
				...
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, 23);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 25);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
		...
		else if (position().z == 27.0f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, 27);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 30);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
		...
		else if (position().z == 30.4f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, 30);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 33);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(10, 0, 38);
		...
		else if (position().z == 33.8f) {
			if (position().x < -4) {
				SteerLib::AgentGoalInfo goal;
				goal.targetLocation = Util::Point(-3, 0, 33);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 35);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(-3, 0, 38);
				_goalQueue.push(goal);
				goal.targetLocation = Util::Point(10, 0, 38);
		...
		if (finalGoal.targetLocation.z >= 0) {
			if (finalGoal.targetLocation.x >= -2.5) {
				...
				_goalQueue.push(goal);
				if (finalGoal.targetLocation.z < 30) {
					goal.targetLocation = Util::Point(-3, 0, 30);
					_goalQueue.push(goal);
				}
				if (finalGoal.targetLocation.z < 25) {
					goal.targetLocation = Util::Point(-3, 0, 25);
					_goalQueue.push(goal);
				}
				if (finalGoal.targetLocation.z < 20) {
					goal.targetLocation = Util::Point(-3, 0, 20);
					_goalQueue.push(goal);
				}
				goal.targetLocation = Util::Point(-2.5f, 0, finalGoal.targetLocation.z);
				_goalQueue.push(goal);
				...
				_goalQueue.push(goal);
				if (finalGoal.targetLocation.z < 30) {
					goal.targetLocation = Util::Point(-3, 0, 29);
					_goalQueue.push(goal);
				}
				if (finalGoal.targetLocation.z < 25) {
					goal.targetLocation = Util::Point(-3, 0, 24);
					_goalQueue.push(goal);
				}
				if (finalGoal.targetLocation.z < 20) {
					goal.targetLocation = Util::Point(-3, 0, 19);
...
Vector SocialForcesAgent::calcGoalForce(Vector _goalDirection, float _dt)
{
	...
	Util::Vector prefForce = (((goalDirection * PREFERED_SPEED) - velocity()) / _dt);// *GOAL_MULT;
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++)
	{
		...
		{
			tmp_agent = dynamic_cast<SteerLib::AgentInterface*>(*neighbor);
			if (id() != tmp_agent->id() && ((*neighbor)->computePenetration(this->position(), this->radius()) > .000001))
			{
				Util::Vector tmp_away_norm = normalize(_position - tmp_agent->position());
				RepulsionForce = RepulsionForce + (tmp_away_norm * tmp_agent->computePenetration(this->position(), this->radius()) * _SocialForcesParams.sf_agent_body_force);
			}
		}
		else
		{
...
Util::Vector SocialForcesAgent::calcWallRepulsionForce(float dt)
{
	...
	*/
	...
	/* The virtual interfarence for any non-agent object in the environment. Inherits SpatialDatabaseItem */
	...
	{
		if (!(*neighbour)->isAgent()) //If the neighbor is not an agent
		{
			tmp_ob = dynamic_cast<SteerLib::ObstacleInterface *>(*neighbour);

			/* ObstacleInterface: Virtual interface for any non-agent object in the environment */
			/* If the object is not an agent, then assign it as a temporary obstacle */

		}
		else /* If the neighbor is an agent, then continue. */
		{
			continue;
		}
		if (tmp_ob->computePenetration(this->position(), this->radius()) > 0.000001)
			/* If the agent is penetrating the obstacle */
		{

			Util::Vector wall_normal = calcWallNormal(tmp_ob); /* Calculate the normal force coming out of the temporary obstacle */
